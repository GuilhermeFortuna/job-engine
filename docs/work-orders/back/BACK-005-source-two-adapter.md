...
Before status becomes `READY`, the coordinator must replace `<SOURCE_TWO_ID>` with the exact CROSS-002 mapping and commit that edit. If this source cannot conform to the accepted adapter contract, stop and propose a separate contract-revision order rather than changing shared files here.

## Owned files

- `/apps/api/src/job_engine/sources/<SOURCE_TWO_ID>.py`
- `/apps/api/src/job_engine/sources/registry.py` (one registration only)
- `/apps/api/tests/sources/fixtures/<SOURCE_TWO_ID>/success.*`
- `/apps/api/tests/sources/fixtures/<SOURCE_TWO_ID>/malformed.*`
- `/apps/api/tests/sources/test_<SOURCE_TWO_ID>.py`
...
cd apps/api && uv run pytest tests/sources/test_contract.py tests/sources/test_<SOURCE_TWO_ID>.py tests/services/test_ingestion.py
git diff --check
```
