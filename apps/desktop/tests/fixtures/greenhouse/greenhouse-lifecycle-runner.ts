...
const testUserDataDir = path.resolve(
  __dirname,
  "..",
  "..",
  "..",
  ".test-userData-greenhouse-life",
);
...
const APPLY_URL = "https://boards.greenhouse.io/acme/jobs/12345";
...
async function main(): Promise<void> {
  ...
  await runCase("claims the owner-selected Greenhouse run", async () => {
  ...
  await runCase("detects the synthetic Greenhouse application form", async () => {
    assert(
      await adapter.detect(context),
      "Greenhouse detect must succeed on the application form",
    );
  });
  ...
  await runCase("observes an unresolved required legal attestation", async () => {
    const pauseUrl = formUrl.replace(
      "/greenhouse/standard",
      "/greenhouse/attestation-pause",
    );
    await view.webContents.loadURL(pauseUrl);
    ...
    assert(
      ...
      "required attestation must be observed unresolved",
    );
    await view.webContents.loadURL(formUrl);
    await context.waitForStable();
  });
  ...
  await runCase("fills authorized visible values", async () => {
    const observation = await adapter.observeStep(context);
    const firstName = observation.fields.find((field) =>
      field.label.toLowerCase().includes("first name"),
    );
    const lastName = observation.fields.find((field) =>
    ...
    assert(
      ...
      "first name, last name, and email must exist",
    );
    const filled = await adapter.fillStep(context, observation, [
      {
        semanticKey: firstName!.semanticKey,
        fieldFingerprint: "fp-first-name",
        value: "Ada",
        checked: null,
        decision: {} as never,
      },
      {
        semanticKey: lastName!.semanticKey,
        fieldFingerprint: "fp-last-name",
        value: "Fixture",
        checked: null,
        decision: {} as never,
      },
  ...
  await runCase(
    "uploads resume and reaches READY_FOR_REVIEW without replaying earlier fills",
    async () => {
      ...
      // The values verified in the previous case must still be the ones on the
      // page: resuming the run must not re-drive completed fills.
      const email = after.fields.find((field) =>
        field.label.toLowerCase().includes("email"),
      );
      const lastName = after.fields.find((field) =>
        field.label.toLowerCase().includes("last name"),
      );
      assert(
        email?.value === "ada.fixture@example.test",
        `earlier verified email must survive, got "${email?.value}"`,
      );
      assert(
        lastName?.value === "Fixture",
        `earlier verified last name must survive, got "${lastName?.value}"`,
      );
  ...
  await runCase("submits once and reconciles a confirmed receipt", async () => {
    ...
    assert(
      ...
      `expected submitted, got ${completed.status}`,
    );
  });
...
app.whenReady()
  .then(main)
  .catch((error) => {
    console.error("Fatal Greenhouse lifecycle fixture error:", error);
    app.exit(1);
  });
