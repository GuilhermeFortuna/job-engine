...
let forms: MockGreenhouseFormServer | null = null;
...
describe("Greenhouse assisted apply against the real backend", () => {
  it("completes fill, upload, review, release, reclaim, and one confirmed submit", async () => {
    forms = new MockGreenhouseFormServer();
    await forms.start();

    const applicationUrl = forms.urlFor("/greenhouse/standard");
    seeded = seedBackend(applicationUrl);
    api = await startApi(seeded);

    const outcome = await runElectronSuite({
      runner: "greenhouse/greenhouse-lifecycle-runner",
      timeoutMs: 180_000,
      env: {
